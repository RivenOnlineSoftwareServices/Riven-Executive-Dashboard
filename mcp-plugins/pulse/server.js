...
async function pulseFetch(path) {
  if (!TOKEN) {
    return { ok: false, status: 0, body: { error: { code: 'no_token', message: 'PULSE_BEARER_TOKEN is not configured. Set it in the Pulse plugin config (the dashboard-2026 bearer).' } } };
  }
  let res;
  try {
